...
import { GET_PRODUCTS_CLIENT, SET_POPULAR_PRODUCT } from "@/actions/product.action"
...
export const AssignPopularProductModal = () => {
    const [search, setSearch] = useState<string>("")
    const [productId, setProductId] = useState<string>("")

    const { open, onClose } = useAssignPopularProduct()
    const searchValue = useDebounce(search)

    const {data:products, isLoading} = useQuery({
        queryKey: ["get-products-to-assign-popular", searchValue],
        queryFn: async () => {
    ...
    const {mutate: setPopularProduct, isPending} = useMutation({
        mutationFn: SET_POPULAR_PRODUCT,
        onSuccess: (data) => {
            setProductId("")
            onClose()
    ...
    return (
        <Dialog open={open} onOpenChange={onClose}>
            <DialogContent>
                <DialogHeader>
                <DialogTitle>Assign popular product</DialogTitle>
                <DialogDescription>
